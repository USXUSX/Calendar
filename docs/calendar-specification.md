...
旅程は全日程を縦の時系列として初期表示する。上部の日付ナビゲーションは表示対象を切り替えるものではなく、該当日へのアンカーとする。各日の日番号・日付・タイトルをまとめた見出し領域全体で、その日だけを折りたたみ／展開できる。個別予定は、分類・アイコン、時刻、予定名と補足の情報階層を保ち、文字を小さくせず余白や重複を減らして iPad mini での密度を調整する。

旅程の分類フィルターは `全て / 移動 / 観光 / 食事 / 宿泊` とし、各予定は `時刻 / 分類 / 本文` の3列を基本とする。日別見出しは日番号・日付と、その日のテーマ・経路概要を2列で表示する。地図の日付操作はアンカーではなくフィルターとし、表示中の日に属する地点・ルート・地点一覧だけを表示する。地点番号は日ごとに1から振り直し、分類フィルターは旅程と共通にする。

準備画面は `準備すること / リオ / 予約・手配` の3セクションで構成する。準備はチェック・日付・項目、予約はチェック・対象日・種類・内容・金額を基本列とする。リオ対象外の旅行ではリオセクションを表示せず、対象旅行では同行または預ける状態と単純な持参品チェック一覧を表示する。

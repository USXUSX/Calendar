...
単一候補、複数候補、複数候補から複数件を選ぶ場合、および選択数自体が未定の場合を扱う。画面上の選択は一時状態であり、元 JSON の `selection` を直接変更しない。AI が次版 JSON を生成し、ユーザーが承認して初めて確定する。
...
同じ場所を予定ごとに複製せず、共通 Place を ID 参照する。レストランの外部評価元は食べログ、ホテルの外部評価元は楽天トラベルとする。評価には値だけでなく評価元と参照時点を保持できるようにし、異なる尺度を一つの共通点数として混同しない。
...
完了した項目も非表示または削除せず、完了状態が分かる形で表示し続ける。画面上のチェックは一時状態として AI への指示に変換し、採用済み JSON を直接書き換えない。
...
標準持参品テンプレートから旅行ごとの項目を作れる。今回不要な項目は削除せず、`notNeeded` 等の状態で不要と明示できるようにする。完了状態と不要状態は区別する。
...
- `id`: 安定 ID
- `category`: accommodation / transport / dining / activity / other 等
- `status`: 未予約、予約済み、変更・取消等を表す状態
- `placeId` または `transportId`: 対象への参照
- `amount`: 金額
- `currency`: 通貨
- `notes`: 予約について AI に伝える指示メモ

費用は宿泊費、交通費、飲食費、体験費、その他などカテゴリ別に集計でき、旅行全体の合計も算出できる。集計値は個々の Booking を根拠に導出し、二重に正本を持たない。

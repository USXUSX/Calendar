...
| 3. 選択経路表示 | 仕様判断待ち（#174） | 明示した主要なエリア間移動だけ描画。経由点編集・全区間自動表示・経路保存なし |
...
2026-09-17のus完了指示に基づきPhase 1を完了。Phase 2の[Issue #167](https://github.com/USXUSX/Calendar/issues/167)も、実装・本番反映・公式同期と実機レビュー後の追加修正を経て、2026-09-18のusによるMac・iPad確認完了の申告と完了指示に基づき完了した。Phase 3と新Goal 3は#174の依頼に基づき続行する。
...
| Phase | 状態 | 成果・境界 |
| --- | --- | --- |
| 1. 地点情報の自動充実 | 生成ガイド改訂案・未完了 | 新規Chat生成で住所・公式URL・短い説明・飲食店の食べログURLと取得可能な評価／確認日。取得不能は空欄。既存Trip一括補完なし |
| 2. 公式URL由来の地点画像 | 実施予定 | 観光地点フォーカス時にog:imageを1枚。保存・キャッシュ・代替・選択UIなし |

#174の範囲は追加のPhase承認を待たず、CAL契約→FRM表示→最小確認→merge・公式同期・必要な本番反映の順で進める。
画像は北海道Tripの代表公式URLで実用性を確認し、実用にならなければ拡大前に停止する。
新Goal 3の終了後は次Goalへ進まず、CAL全体の成果・未確認事項・実用性を振り返ってusへ報告する。

着手調査でGoogle Routes APIが日本の公共交通に非対応であることを確認したため、鉄道2区間の表示方式をusが判断するまで#174は停止する。準備済み契約はDraftであり、本番反映・実Trip設定は未実施。

...
| Phase | 状態 | 成果・境界 |
| --- | --- | --- |
| 1. 旅程連動地図を表示 | 完了／us振り返り待ち | Google Mapsで日別・全日、Pin詳細、旅程フォーカス連動。実経路線・地点編集なし |
...
座標未登録地点は件数表示し、補正は行わない。実Tripの編集操作・物理端末受入は未実施。
追加した永続状態・地図保存API・経路providerはない。usはPhase 1の実用性を振り返り、
Phase 2（地点確認・補正）へ進むか判断する。Phase 2は未着手。

### Phase 1実用調整（Calendar #162 / Frame #106）

地点集約・対象整理、位置登録済みだけの連番、地点名Pin、コンパクト一覧、吹き出し集約、選択範囲とiPad縦配置を調整する。全体レイアウト・地図高はus実機レビューで受入済み。#162追加指示により候補全件・枝番・衝突優先度・2列一覧・詳細開閉を最終調整し、再提示で停止する。Phase 2の地点補完は要件記録のみ。

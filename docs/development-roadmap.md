...
1. **完了**: AIG接続境界と最小generation stateを確定する。
2. CALにWorking Tripごとの最新generation stateとcandidateを実装する。
3. AIGにWorking exportからcomplete candidateを1件返すstatelessな最小生成境界を実装する。
4. CAL → AIG → Phase 5 candidate受入れを接続する。
5. `auto / review` adoption policyを実装する。
6. FRMに生成開始と`generating / failed / candidate_ready / adopted`の最小表示・操作を追加する。

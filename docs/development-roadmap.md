...
| 3. 地点情報・公式画像を実用化 | 新規旅程の手修正を減らし観光地点を把握する | 生成時の情報充実とフォーカス時の公式画像を使える | 取得不能は空欄／非表示。既存Trip一括補完・画像保存・汎用検索基盤なし |
...
現在の範囲では全面再構築は不要。メール取込、旧Working／OpenAI経路の追加開発、[AIG #12の保持性調査](https://github.com/USXUSX/AI-Gateway/issues/12)は保留し、次の旅行等で必要が確認された場合に再判断する。全ケース対応や初期rule検出外の先回り実装は行わない。既存データや旧基盤の削除・移行は行わず、通常利用の支障だけを個別Issueで直す。
...
| 3. 選択経路表示 | 完了（#174） | 明示した主要な車区間だけ描画、鉄道はGoogle Maps経路検索リンク。経由点編集・全区間自動表示・経路保存なし |
...
2026-09-17のus完了指示に基づきPhase 1を完了。Phase 2の[Issue #167](https://github.com/USXUSX/Calendar/issues/167)も、実装・本番反映・公式同期と実機レビュー後の追加修正を経て、2026-09-18のusによるMac・iPad確認完了の申告と完了指示に基づき完了した。Phase 3と新Goal 3も#174で完了した。
...
地図保存API・経路providerは追加していない。このPhase 1完了時点ではPhase 2は未着手。Phase 2の記録は#167を参照する。
...
仕様は[座標契約](map-locations.md)。関連Frame Issueは[Frame #114](https://github.com/USXUSX/Frame/issues/114)。地点の表示名、コメントの一括保存、候補Pinの表示優先度、Mac・iPadの操作配置を実機レビューに基づき調整した。2026-09-18、usからMac・iPadとも問題なしとの確認を受け、Phase 2を完了した。この時点ではPhase 3へ進まず、後続の#174で選択経路表示を実施した。

## Goal 3: 地点情報・公式画像を実用化する

#174により旧Goal 3（旅行Todo）・旧Goal 4（旅行記録）は保留として現行ロードマップから外す。機能・データの変更はしない。

| Phase | 状態 | 成果・境界 |
| --- | --- | --- |
| 1. 地点情報の自動充実 | 完了（#174） | 新規Chat生成で住所・公式URL・短い説明・飲食店の食べログURLと取得可能な評価／確認日。取得不能は空欄。既存Trip一括補完なし |
| 2. 公式URL由来の地点画像 | 完了（#174） | 観光地点フォーカス時にog:imageを1枚。保存・キャッシュ・代替・選択UIなし |

2026-09-18、us承認の車2区間の実経路と鉄道2区間のGoogle Maps経路検索リンクを本番反映した。エリア内移動は非表示。新規生成の代表2地点を実際の検索情報で作成し、合成日程のSchema・semantic・一時DB取込で確認した。JSON取込の座標・Google Place ID補完は維持する。

北海道Tripの公式URLでは赤れんが庁舎の画像を実取得・表示できた。og:imageなし、画像でない応答、取得失敗は非表示となる。全地点の画像を保証せず、保存・キャッシュ・代替取得を足さずに現行範囲を完了する。

公開画面はMac 1280×900とiPad相当744×1133で確認。今回の物理iPad受入は未実施であり、Phase 2時のus確認とは区別する。CAL/FRMのtest・merge・公式同期・既存Frameへの本番反映を完了した。実Trip変更は承認済み4区間の表示指定だけで、その他のeffective値と正式JSONを保持した。

### CAL全体の振り返りと終了境界

Goal 1のChat生成・取り込み・直接編集・継続往復・候補・天気に、Goal 2の地点地図・位置補正・主要区間確認と、Goal 3の生成時の地点情報充実・取得可能な公式画像が加わり、旅程の作成から旅行中の場所確認までを既存経路で扱えるようになった。
鉄道はGoogle Mapsへの遷移、画像は公式側の提供状況、地点情報は検索で確認できる範囲に依存する。この制約を汎用検索基盤や代替provider、画像保存で吸収せず、現在の用途に必要な範囲で開発を区切る。
次Goalへ進まない。旅行Todo・旅行記録の機能／データは変更せず、現行ロードマップ外の保留とする。実利用で具体的な支障が出たときだけ保守対象として判断する。

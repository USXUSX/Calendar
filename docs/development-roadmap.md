...
| 2. 地点確認・補正 | 実装済み・us画面確認待ち | JSON取込時の不足座標補完、編集モードで位置設定・Pin修正・保存 |
...
2026-09-17のus完了指示に基づきPhase 1を完了。その後の[Issue #167](https://github.com/USXUSX/Calendar/issues/167)実行依頼によりPhase 2の座標補完・位置修正へ進む。実装・本番反映・公式同期後に画面を提示し、us確認待ちで停止する。Phase 3は未着手。
...
取り込み時に地図表示対象の不足座標だけを一度検索し、先頭結果をCALへ保存する。既存座標は保持し、検索失敗でも取り込みを完了する。地図編集では位置を設定・修正・取消できる。予定追加は既存地点の座標を利用し、新地点を座標なしで保存できる。
仕様は[座標契約](map-locations.md)。関連Frame Issueは[Frame #114](https://github.com/USXUSX/Frame/issues/114)。本番反映後の実画面でusの実用性確認を待ち、Phase 3へは進まない。

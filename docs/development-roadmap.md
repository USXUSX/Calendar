...
| 2. 地点確認・補正 | 未着手・us指示で停止 | 共通位置設定でエリア・施設検索、位置確認、Pin微調整・保存 |
...
2026-09-17のus完了指示に基づきPhase 1を完了とする。Phase 2には進まず停止し、再開は別途usの指示を待つ。
...
地図保存API・経路providerは追加していない。Phase 2（地点確認・補正）は未着手のまま停止する。

### Phase 1実用調整（Calendar #162 / Frame #106）

地点集約・対象整理、位置登録済みだけの連番、候補全件と枝番、地点名Pinの衝突優先度、2列一覧、吹き出しの予定内容・通常コメント・開閉、選択範囲とiPad縦配置を完了。中間日の移動拠点は最初の到着順、最終日は帰路の出発順とした。関連test・CI、本番反映・実画面確認・公式同期を完了し、2026-09-17のus完了指示により再確認待ちを解消した。確認結果とPRは[Issue #162](https://github.com/USXUSX/Calendar/issues/162)に記録。Phase 2の地点補完は要件記録のみ。

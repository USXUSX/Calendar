...
| 2. UIに合わせて旅程データ・表示モデルを整理 | 現在 | Phase 1の完成像を、既存の旅程正本とCAL境界から無理なく表示できるようにする | 画面に必要な情報、状態、関係を表示モデルから取得でき、不足や変換規則が明確になっている | formal Trip JSON、SQLite、effective Tripの正本境界を維持し、UI都合の二重正本や将来向けの過剰な汎用化を作らない |
| 3. 直接編集を完成 | 未着手 | usが具体的な値を編集画面で補正できる実用的な経路を完成する | 予定選択から端末別編集画面、Validation、反映状態の確認まで一つの流れで行える | CALの意味ベースのcommandとDirect Overrideを通し、SQLiteやTrip JSONを画面から直接変更しない。曖昧な変更意図はAI指示と分ける |
| 4. AI指示による局所更新を完成 | 未着手 | 編集対象に紐づく自然言語の変更意図を、旅程全体を壊さず反映できるようにする | 編集画面下部の指示受付から候補検証・採用または修正可能な失敗状態までを扱える | 現行のCAL-owned Patch、complete candidate Validation、atomic adoptionを維持する。provider固有処理、model、credentialはCAL coreへ入れず、直接編集を置き換えない |
...
### 現在のPhase 2: UIに合わせて旅程データ・表示モデルを整理する

1. 確定UIに必要な表示項目を、formal Trip JSON、SQLite、effective Trip、派生表示のどこから得るか棚卸しする。
2. 確定、暫定、未定、候補、通常コメント、重要コメント、天気、カテゴリーの表示状態と変換規則を整理する。
3. 候補追加と `OK / NG`、予定に紐づくAI指示を、採用済み旅程と混同しない入力・一時状態として整理する。
4. 編集画面の具体値入力を、既存の意味ベースcommandとDirect Overrideへ対応付ける。
5. 合成Tripで表示モデル契約を検証し、Phase 1の確定UIを無理なく構成できることを確認する。
6. Phase 2の結果を確認し、Phase 3以降の実装範囲を見直す。

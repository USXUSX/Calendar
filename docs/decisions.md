...
- 2026-08-22, Issue #29: local real-trip viewing uses a Python-standard-library server bound only to `127.0.0.1`. Its initial read-only API exposes trip-list metadata and adopted `current.json`; `history/` and a general `Calendar_Local` static path remain unavailable. Static review hosting keeps the synthetic sample fallback. Candidate access is extended explicitly by Issue #31 below.
- 2026-08-22, Issue #31: the loopback API explicitly exposes `current.json` for normal viewing and `candidate.json` for unadopted review, while `history/` and all write methods remain unavailable. AI update requests package the unnormalized source `current.json`, instructions, and UI update materials together; candidate mode is visibly read-only.

...
| 情報の所有箇所を一つにする | 同一情報を複数箇所へ冗長に複製せず、最も適切な所有箇所へ置く。予約に属する情報と予定・施設固有の情報は前節の格納先に分け、通常コメントと重要コメントにも同じ内容を重ねない。共通情報は参照で共有する。宿泊予約情報を同日の別予定（例: 宿で夕食）の重要コメントへ複製しない |
| 構造と説明を分ける | 構造化フィールドで表現済みの情報を説明文へ重複させず、説明文にはそこで分からない判断条件・補足だけを書く。候補名の列挙や時刻・金額の再掲でコメントを埋めない。自然な予定本文に採用済みPlace名を含める契約は維持する。予約前の備忘・検討事項は原則として旅程表示用コメントへ入れない。金額は必要ならBooking.amount等の構造化fieldへ保持し、通常コメント・重要コメント・補足説明には原則記載しない。既存の確認済み金額を消すこと自体は目的にしない |

...
候補行の正式採用は`selection: [place_id]`だけを保存する。旧ng値は保持できるが判断UIには表示しない。
...
既存undecidedもUIでは未確定にまとめる。滞在時間表示もチェックで直接選ぶ。
...
候補行の「正式採用 / 選択解除」はselectionだけを即時保存する明示操作で、本文・状態・いいねを書き換えない。

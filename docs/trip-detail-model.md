...
訪問予定を優先して並べる。移動参照だけの拠点は、最終日以外で到着と後続出発の両方がある場合、最初の到着順に置く。それ以外は最後の出発順（出発がなければ最初の参照順）に置く。最終日は市場の食事・駅周辺昼食・帰路の駅・空港の順を維持する。座標なしもnullのまま返し、番号はFrameが座標あり群だけに詰めて振る。候補群は共通の訪問番号と個別の枝番を使い、位置未登録候補も一件ずつ一覧へ出す。非対象予定は座標があっても表示しない。
...
地図対象の位置未登録地点を抽出し、住所＋名称、なければ名称＋エリアで検索する。個別・一括の確認を経て既存CAL地点補完の採用境界へ渡し、既存座標は上書きしない。エリア・観光・宿泊の共通UIを検討する。旅程作成時に確実な位置を含め、取り込み時に不足を補完する流れもこのPhaseで扱う。#162では検索・補完・実データの座標保存を実装しない。

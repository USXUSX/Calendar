...
class ChatPasteTests(unittest.TestCase):
    ...
    def corrected(self, text=None):
        review = self.domain.parse_chat_paste(text or self.example)
        for day in review["draft"]["days"]:
            for item in day["items"]:
                if item["kind"] == "schedule":
                    item["category"] = "food" if item["title"] == "昼食" else "sightseeing"
        return review
    ...
    def test_database_failure_rolls_back_new_file(self):
        ...
        with self.assertRaises(ValidationError):
            self.domain.import_chat_paste("db-failure", self.corrected(), confirmed=True)
        self.assert_empty()

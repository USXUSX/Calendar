...
package_file="$repo_root/Sources/web/update-package.mjs"
...
grep -Fq '対象種別: ${material.type}' "$package_file"
grep -Fq '安定ID: ${material.id}' "$package_file"
...
grep -Fq 'createUpdatePackage(view.originalJson, trip, materials)' "$app_file"
grep -Fq 'AI更新依頼をコピー' "$app_file"
